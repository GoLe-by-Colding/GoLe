...
| 작업을 push·PR 로 내보낼 때 | 볼트 `10_개발일지/` — 오늘자 내 일지 (훅이 강제한다) |
...
| 제품·사업 기획 결정이 미뤄졌다 | `01_기획/기획 갭 목록.md` (2026-08-29 스냅샷) |
| 운영·인프라 미결이 생겼다 | `08_개선과제/알려진 개선 과제.md` 의 "운영·인프라 미결" |
| 운영에서 문제가 났다 | `07_이슈기록/` |
| 작업을 push·PR 로 내보냈다 | `10_개발일지/<이름>/YYYY-MM-DD/YYYY-MM-DD.md` (아래) |

- 파일 앞에 `tags` · `created` · `updated` 프런트매터를 넣는다. 기존 문서를 열어 형식을 맞춘다.
- **미뤄둔 결정은 개발일지에만 적고 끝내지 않는다.** 일지는 수명이 며칠이라 하루 만에
  묻힌다. 며칠 넘게 살아 있을 미결은 `08_개선과제` 로 올린다.
...
`06_개발로그`가 "이걸 **왜 이렇게** 만들었나"라면, `10_개발일지`는 "지금 **어디까지**
돼 있나"다. 셋이 각자 에이전트를 돌리므로, 말로 인수인계하는 대신 여기를 읽게 한다.

```text
10_개발일지/<이름>/2026-09-12/2026-09-12.md   # 요약. <이름> = 가원 · 승찬 · 수민
10_개발일지/<이름>/2026-09-12/인프라.md       # 덩어리가 있을 때만
```

- **사람 폴더가 먼저 갈리고 그 밑에 날짜 폴더**다. 셋이 같은 날 동시에 커밋해도 충돌하지
  않는다. 하루에 여러 번 작업하면 그날 폴더 안을 고친다 — 새 폴더를 만들지 않는다.
- **요약 파일은 폴더와 같은 이름**을 쓰고 반드시 있어야 한다. 숫자로 시작해 목록 맨 위에
  오므로 인공 접두사가 필요 없다. 이어받는 사람이 이것 하나만 읽어도 상황이 서게 쓴다.
  네 칸이다 — **오늘 한 것**(PR·커밋 번호 필수) / **지금 열려 있는 것** /
  **이어받는 사람이 알아야 할 것** / **건드린 경로**.
- **주제 파일은 덩어리가 있을 때만** 만든다. 짧은 명사 하나로 —
  `인프라` `백엔드` `프론트` `운영이슈` `CI` `문서` `기타`.
  작은 날은 요약 하나로 끝낸다. 빈 주제 파일을 깔면 폴더만 늘고 읽히지 않는다.
- 날짜·사람은 폴더가 이미 말하므로 파일명에 넣지 않는다. 대신 이름이 날짜마다 겹치므로
  **위키링크는 전체 경로로** 건다 — `[[10_개발일지/가원/2026-09-12/인프라]]`.
- **"건드린 경로"를 빼먹지 않는다.** 같은 모노레포에서 셋이 동시에 돌리므로 경로가
  겹치면 서로의 편집을 덮어쓴다. 세션을 나눌 때 이 칸을 먼저 본다.
- 다 썼으면 `10_개발일지/<이름>/README.md`의 날짜 목록 맨 위에 한 줄 더한다.
- 기능 한 편을 끝냈으면 `06_개발로그`와 **둘 다** 쓴다. 설계 근거는 저쪽, 상태는 이쪽.

`.claude/hooks/devlog-guard.sh`(Stop 훅)가 강제한다. `git push` · `gh pr create` ·
`gh pr merge` 를 한 세션은 그날 폴더의 요약 파일이 없으면 턴을 끝내지 못한다.
커밋마다 걸면 시끄러워서 **작업이 밖으로 나가는 순간**으로 잡았다. 볼트를 안 받아뒀거나 커밋 이메일이
팀원 목록에 없으면 막지 않는다 — 볼트 위치는 `GOLE_VAULT`, 이름은 `GOLE_MEMBER`로 덮는다.
...
- **머지 방식은 방향마다 다르다**(2026-09-13 개정).
  - 작업 브랜치 → `dev` 는 **머지 커밋**(`gh pr merge --merge`). 레이어별로 나눈 커밋이
    `dev`에 그대로 남아야 나중에 `git blame`·`git log -S`로 "이 줄이 왜 이렇게 됐나"를
    추적할 수 있다. 에이전트가 맥락을 복원할 때 읽는 것이 이 이력이다.
    squash를 쓰면 "레이어별로 커밋을 분리한다"는 아래 규칙이 값을 못 받고 버려진다.
  - `dev` → `main` 릴리스는 **squash**(`gh pr merge --squash`). `main`의 커밋 하나 =
    릴리스 하나. `main` 보호에 `required_linear_history`가 걸려 있어 머지 커밋은
    GitHub이 **거부한다** — 선택이 아니라 제약이다.
- 그래서 `dev`에는 머지 커밋·`chore(sync)` 역병합이 섞이고, `main`에는 squash만 남는다.
  GitKraken 그래프가 복잡해 보이는 건 이 구조 때문이지 사고가 아니다.
...
- 백엔드/프론트는 레이어별로 커밋을 분리한다. `dev` 머지가 머지 커밋이라 이 분리가
  그대로 남는다 — 나중에 그 줄을 추적하는 것은 사람이 아니라 에이전트다.
