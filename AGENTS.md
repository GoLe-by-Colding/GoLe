...
| 운영에서 문제가 났다 | `07_이슈기록/` |
| 작업을 push·PR 로 내보냈다 | `10_개발일지/<이름>/YYYY-MM-DD/00_오늘 한 것.md` (아래) |
...
```text
10_개발일지/<이름>/YYYY-MM-DD/00_오늘 한 것.md   # <이름> = 가원 · 승찬 · 수민
10_개발일지/<이름>/YYYY-MM-DD/인프라.md          # 덩어리가 있을 때만
```

- **사람 폴더가 먼저 갈리고 그 밑에 날짜 폴더**다. 셋이 같은 날 동시에 커밋해도 충돌하지
  않는다. 하루에 여러 번 작업하면 그날 폴더 안을 고친다 — 새 폴더를 만들지 않는다.
- **`00_오늘 한 것.md`는 반드시 있어야 한다.** 이어받는 사람이 이것 하나만 읽어도 상황이
  서게 쓴다. 네 칸이다 — **오늘 한 것**(PR·커밋 번호 필수) / **지금 열려 있는 것** /
  **이어받는 사람이 알아야 할 것** / **건드린 경로**.
- **주제 파일은 덩어리가 있을 때만** 만든다(`인프라` `백엔드` `프론트` `운영이슈` `기타`).
  작은 날은 `00_` 하나로 끝낸다. 빈 주제 파일을 깔면 폴더만 늘고 읽히지 않는다.
...
`gh pr merge` 를 한 세션은 그날 폴더의 `00_오늘 한 것.md`가 없으면 턴을 끝내지 못한다.
