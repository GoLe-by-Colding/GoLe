...
| 운영에서 문제가 났다 | `07_이슈기록/` |
| 작업을 push·PR 로 내보냈다 | `10_개발일지/YYYY-MM/YYYY-MM-DD_<이름>.md` (아래) |
...
```text
10_개발일지/YYYY-MM/YYYY-MM-DD_<이름>.md     # <이름> = 가원 · 승찬 · 수민
```

- **하루에 파일 하나, 사람마다 따로.** 한 파일에 여럿이 쓰면 셋이 동시에 커밋할 때 충돌한다.
  하루에 여러 번 작업하면 같은 파일을 덧붙여 고친다.
...
`gh pr merge` 를 한 세션은 오늘자 자기 일지가 없으면 턴을 끝내지 못한다. 커밋마다 걸면
시끄러워서 **작업이 밖으로 나가는 순간**으로 잡았다. 볼트를 안 받아뒀거나 커밋 이메일이
