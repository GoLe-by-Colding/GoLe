...
DAY=$(date +%Y-%m-%d)
# 배치: 10_개발일지/<이름>/YYYY-MM-DD/YYYY-MM-DD.md
# 요약 파일은 폴더와 같은 이름을 쓴다 — 숫자로 시작해 목록 맨 위에 오고, 인공 접두사가 필요 없다.
# 사람 폴더가 먼저 갈리므로 셋이 같은 날 동시에 커밋해도 충돌하지 않는다.
DAYDIR="$VAULT/10_개발일지/$WHO/$DAY"
LOG="$DAYDIR/$DAY.md"

# 있고 형식만 붙여넣은 게 아니면(빈 칸 표시가 남아 있지 않으면) 통과.
...
jq -n --arg log "$LOG" --arg dir "$DAYDIR" --arg who "$WHO" --arg vault "$VAULT" --arg what "$WHAT" '{
  ...
  reason: ("이 세션에서 작업을 밖으로 내보냈는데(아래) 오늘자 개발일지가 없다.\n" + $what +
    "\n" + $dir + "/ 를 만들고 그 안에\n" + $log + " 를 써라.\n" +
    "요약 파일은 폴더와 같은 이름이다. 형식은 " + $vault + "/10_개발일지/README.md 에 있다.\n\n" +
    "네 칸이다 — 오늘 한 것(PR·커밋 번호 필수) / 지금 열려 있는 것 / 이어받는 사람이 알아야 할 것 / 건드린 경로.\n" +
    "\"건드린 경로\"는 빼먹지 마라. 셋이 같은 모노레포에서 각자 에이전트를 돌리므로 경로가 겹치면 서로 덮어쓴다.\n" +
    "덩어리가 있으면 같은 폴더에 주제 파일(인프라·백엔드·프론트·운영이슈·CI·문서·기타)을 더한다. 작은 날은 요약 하나로 끝낸다.\n" +
