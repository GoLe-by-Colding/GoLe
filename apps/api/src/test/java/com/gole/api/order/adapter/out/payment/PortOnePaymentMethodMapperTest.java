...
class PortOnePaymentMethodMapperTest {
    ...
    /**
     * V2 결제 조회 응답의 실제 모양이다({@code method.provider}).
     * {@link PortOnePaymentGatewayAdapter}의 원장 검증이 읽는 경로와 같아야 한다 —
     * 검증과 기록이 갈라지면 "검증은 통과했는데 결제수단은 UNKNOWN"이 생긴다.
     */
    @Test
    void easyPay_extractsProviderFromV2ResponseShape() {
        PaymentMethod method = fromMethod(Map.of("type", "EASY_PAY", "provider", "KAKAOPAY"));
