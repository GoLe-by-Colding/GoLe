...
public interface SocialLoginUseCase {
    ...
    /** provider 동의 화면 URL을 만든다. 서버가 복귀 경로를 검증하고 state와 함께 저장한다(CSRF). (S4) */
    AuthorizeUrlResult authorizeUrl(
    ...
    /** provider URL과 요청 브라우저에 HttpOnly 쿠키로 결박할 일회성 state를 함께 돌려준다. */
    record AuthorizeUrlResult(String url, String state) {}
