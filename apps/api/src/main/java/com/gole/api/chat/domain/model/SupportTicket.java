...
public record SupportTicket(
    ...
    public SupportTicket(
            ...
            Instant updatedAt,
            Instant resolvedAt) {
        this(roomId, requesterId, status, assigneeId, createdAt, updatedAt, resolvedAt, 0L);
    }
    ...
    public static SupportTicket opened(String roomId, String requesterId, Instant now) {
        return new SupportTicket(roomId, requesterId, SupportStatus.UNASSIGNED, null, now, now, null, 0L);
    }
    ...
    public SupportTicket assignTo(String agentId, Instant now) {
        ...
        return new SupportTicket(
                roomId, requesterId, SupportStatus.IN_PROGRESS, agentId, createdAt, now, resolvedAt, version);
    }
    ...
    public SupportTicket transferTo(String agentId, Instant now) {
        ...
        return new SupportTicket(roomId, requesterId, status, agentId, createdAt, now, resolvedAt, version);
    }
    ...
    public SupportTicket agentReplied(Instant now) {
        ...
        return new SupportTicket(
                roomId, requesterId, SupportStatus.WAITING_USER, assigneeId, createdAt, now, resolvedAt, version);
    }
    ...
    public SupportTicket userReplied(Instant now) {
        if (status == SupportStatus.UNASSIGNED) {
            ...
            // 가라앉지 않는다.
            return new SupportTicket(roomId, requesterId, status, assigneeId, createdAt, now, resolvedAt, version);
        }
        if (status == SupportStatus.RESOLVED) {
            return reopen(now);
        }
        return new SupportTicket(
                roomId, requesterId, SupportStatus.IN_PROGRESS, assigneeId, createdAt, now, resolvedAt, version);
    }
    ...
    public SupportTicket resolve(Instant now) {
        ...
        return new SupportTicket(roomId, requesterId, SupportStatus.RESOLVED, assigneeId, createdAt, now, now, version);
    }
