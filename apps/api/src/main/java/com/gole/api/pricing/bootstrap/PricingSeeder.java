...
/**
 * 시세 시드 데이터. 컬렉션 전체가 비어 있을 때만 인기 세트의 체결 이력을 적재한다(멱등).
 * 최근 약 24주에 걸쳐 완만한 변동을 갖는 거래를 생성해 시세 차트가 의미를 갖게 한다.
 *
 * <p>체결 이력이 한 건이라도 있으면 아무것도 수정하지 않는다. 실거래는 시세의 원천이므로
 * 샘플 시더가 삭제·재매핑·보충해서는 안 된다. 레거시 변환은 별도 명시적 마이그레이션으로만 수행한다.
 */
@Component
@Order(3)
@ConditionalOnProperty(name = "gole.pricing.seed-on-empty", havingValue = "true")
public class PricingSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        if (repository.count() > 0) {
            log.info("[seed] pricing: 기존 체결 이력이 있어 샘플 적재를 건너뜀");
            return;
        }
        ...
        if (!docs.isEmpty()) {
            log.info("[seed] pricing: 빈 컬렉션에 샘플 {}건 적재(등급별)", docs.size());
        }
    }
