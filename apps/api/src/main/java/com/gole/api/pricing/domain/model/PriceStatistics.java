...
public record PriceStatistics(
        String setNumber, long latestPrice, long highestPrice, long lowestPrice, int transactionCount) {
    ...
    public static PriceStatistics from(String setNumber, List<PriceTransaction> recentFirst) {
        ...
        long highest =
                recentFirst.stream().mapToLong(PriceTransaction::price).max().orElse(latest);
        long lowest =
                recentFirst.stream().mapToLong(PriceTransaction::price).min().orElse(latest);
