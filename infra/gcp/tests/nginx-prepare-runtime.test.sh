...
grep -Fqx 'resolver 127.0.0.11 valid=10s ipv6=off;' "$candidate_http"
grep -Fq 'server backend:8080 resolve;' "$candidate_http"
grep -Fq 'server frontend:3000 resolve;' "$candidate_http"
grep -Fq 'proxy_pass http://gole_backend;' "$candidate_http"
grep -Fq 'proxy_pass http://gole_frontend;' "$candidate_http"
grep -Fq '"$request_method $uri $server_protocol"' "$candidate_http"
! grep -Fq '$request ' "$candidate_http"
! grep -Fq '$args' "$candidate_http"
! grep -Eq 'proxy_pass http://(backend|frontend):' "$candidate_http"
...
grep -Fqx 'resolver 127.0.0.11 valid=10s ipv6=off;' "$candidate_https"
grep -Fq 'zone gole_backend 64k;' "$candidate_https"
grep -Fq 'server backend:8080 resolve;' "$candidate_https"
grep -Fq 'zone gole_frontend 64k;' "$candidate_https"
grep -Fq 'server frontend:3000 resolve;' "$candidate_https"
grep -Fq 'proxy_pass http://gole_backend;' "$candidate_https"
grep -Fq 'proxy_pass http://gole_frontend;' "$candidate_https"
grep -Fq '"$request_method $uri $server_protocol"' "$candidate_https"
! grep -Fq '$request ' "$candidate_https"
! grep -Fq '$args' "$candidate_https"
! grep -Eq 'proxy_pass http://(backend|frontend):' "$candidate_https"
