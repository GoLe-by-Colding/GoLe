...
Terraform은 VM, 고정 IP, 방화벽을 만들고 시작 스크립트가 Ubuntu 호스트에 Docker,
MongoDB, Redis, MinIO, Nginx, Java, Node, pnpm, PM2를 설치한다.
...
gcloud compute ssh gole-production --zone asia-northeast3-a -- \
  sudo certbot --nginx -d gole.co.kr -d www.gole.co.kr \
  --non-interactive --agree-tos -m coldingcontact@gmail.com --redirect
```
