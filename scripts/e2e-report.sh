#!/usr/bin/env bash
# GoLe E2E 실행 + 리포트를 gole.co.kr/test-report/에 배포
# 사용: bash scripts/e2e-report.sh [local|live]
#   local  — 로컬 dev 서버 대상 (기본)
#   live   — https://gole.co.kr 대상 (read-only, 쓰기 플로우 skip)
...
if [ "$MODE" = "live" ]; then
  E2E_BASE_URL=https://gole.co.kr pnpm --filter web exec playwright test --reporter=html || true
else
...
echo "✓ 리포트 공개: https://gole.co.kr/test-report/"
