...
- [~] 13.5 알림 — `notification` 컨텍스트, 헤더 벨·`/notifications`, 주문·배송·분쟁·댓글·팔로우·직거래
      트리거와 **팔로우 셀러 신규 매물** 알림까지 구현했다(스펙 `.kiro/specs/notifications/`).
      **위시리스트 가격 변동**은 현재 매물 가격 수정 유스케이스 자체가 없어 후속으로 남긴다.
