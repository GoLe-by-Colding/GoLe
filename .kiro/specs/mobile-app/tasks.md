...
시뮬레이터에서 실제 데이터로 확인함(2026-09-05): 검색 18건·필터·정렬, 매물 상세의 구성/하자 고지,
세트 10294 시세(최근 ₩492,800 · 거래 63건), 커뮤니티 피드, 알림함 비로그인 안내.

`[~]` 는 항목의 일부만 끝난 것이다. 남은 것은 각 줄에 적었다.


- [x] 6.1 홈 — 트렌딩 세트를 코어 API로 조회 (R5.1)
- [~] 6.2 검색 + 카테고리·정렬 필터 완료. **페이지네이션·무한 스크롤은 미구현** —
      `searchListings` 가 커서 없이 전체를 돌려주므로 코어 API 부터 넓혀야 한다 (R5.1, R5.2)
- [x] 6.3 매물 상세 + 갤러리, `thumbnailUrl` 공유 (R5.1, R5.3)
- [x] 6.4 세트 상세·시세, 시세 탐색 (R5.1)
- [~] 6.5 셀러샵 완료. 프로필은 내 정보 탭이 대신한다. **컬렉션 미구현** (R5.1)
- [~] 6.6 커뮤니티 목록 완료. **글 상세·댓글 미구현** (R5.1)
- [~] 6.7 알림함 완료(읽음 처리 포함). **주문 상세 미구현** — 결제 동선과 함께 다룬다 (R5.1)
