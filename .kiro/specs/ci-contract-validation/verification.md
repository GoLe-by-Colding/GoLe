...
- 후속 실행 #34852259231에서는 복구 테스트가 통과했으나 heartbeat 테스트의 `entered.wait(2)`가 실패함(96 passed / 1 failed). 로그만으로 진입 지연과 조기 임대 만료를 확정적으로 구분할 수는 없음.
- 일반 fixture에서 불필요한 0.3초 임대 설정을 제거하고 운영 기본값을 사용함. heartbeat 검사는 별도 고정 시계·0.3초 임대로 실제 갱신을 관찰한 뒤 기존 만료 시각을 넘기도록 바꿈. sleep 기반 추정을 제거하며 갱신이 없으면 반드시 실패함.
- 최종 로컬 파일 전체: 36 passed / 0 failed / 0 skipped, 14.08초. 원격 최종 결과는 PR에 누적 기록해 문서 갱신만으로 재실행을 반복하지 않음.
