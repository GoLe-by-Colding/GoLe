...
- **건너뛴 커밋은 다음 실행의 후보에서 빠진다**(D11). 그리고 **건너뛴 커밋보다 오래된 후보는
  살아남는다** — 제외는 `continue` 이지 `break` 가 아니다. 이 두 번째 항목이 본체다.
- **실패한 커밋은 홍보 경계 너머에 있어도 다음 실행에서 되살아난다**(D11). 원장이 없으면
  스캐너가 경계에서 멈춰 그 실패가 영영 사라진다 — 이 회귀가 본체다.
- **턴 예산을 다 쓴 중단은 `skipped` 가 아니라 `deferred` 로 기록된다**(D11). 판단과 중단을
  같은 칸에 넣으면 결론이 안 난 릴리스가 영구 제외된다.
- 세션 주입 스크립트는 토큰에 따옴표가 섞여도 깨지지 않고, 허용한 필드만 브라우저에 심으며,
  토큰이 비면 익명으로 조용히 넘어가지 않고 실패한다(D12/D14).
- 세션 공급자가 없으면 카메라는 익명으로 동작한다 — 드라이런이 백엔드 없이 살아야 한다(D16).
- `/profile` `/notifications` 는 캡처 허용목록에서 빠지고, `/sell` `/brick-filter` 처럼
  로그인이 필요하지만 개인 정보가 아닌 기능 화면은 들어온다(D12/D19).
