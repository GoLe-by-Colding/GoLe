...
### D20. 관리자 콘솔 발 요청을 큐로 받고, 실행은 여전히 타이머가 한다

타이머 발 실행만으로는 **방금 배포한 릴리스를 바로 올리거나 실패한 실행을 다시 돌릴 방법이
없다.** 다음 날까지 기다려야 한다. 그래서 관리자 콘솔에 초안 생성 요청을 붙인다.

**백엔드는 에이전트를 띄우지 못한다.** backend 컨테이너에 docker socket·sudo·hostctl·바인드
마운트가 전부 없고, `infra/gcp/tests/test_production_compose_policy.py` 의
`test_rejects_docker_escape_settings` 가 docker.sock 마운트·privileged·host-network 를 **회귀
테스트로 금지**한다. 게다가 실효 메모리 여유가 ~900MB 인데 홍보 컨테이너는 1g 라 동시 2개가
뜨면 OOM 이고, 한 실행이 최장 45분이라 HTTP 요청-응답 안에서 끝낼 수도 없다.

그래서 **요청은 큐에 쌓고 기존 타이머 실행이 비운다.** 즉시성은 T12 로 미룬다. 그 동안에도
기능은 성립한다 — 관리자가 요청하면 다음 실행이 그것을 **먼저** 처리한다.

**판단의 주인은 Java 하나다.** "이 릴리스를 홍보해도 되는가"는 `PromotionPost` 의 점유
(`claimedSourceCommitSha`, D2)와 3회 상한(D11)이 이미 답하고 있다. 요청 애그리거트는 그
규칙을 복사하지 않고 접수 시점에 **물어보기만** 한다 — 두 벌이 되면 "반려로 점유가 풀렸는데
요청은 못 만드는" 어긋남이 생긴다.

반대로 **Python 의 로컬 휴리스틱은 지정 실행에서 전부 건너뛴다.** manifest 원장(건너뜀·재시도),
7일 탐색 창, `MAX_WALK_COMMITS` 가 그것이다. 이것들은 자동 선정을 위한 장치이고, 관리자가
커밋을 콕 집은 상황에서 다시 거르면 후보가 0건이 되어 **관리자 눈에는 아무 일도 일어나지 않은
것처럼 보인다** — 이 기능이 없애려는 바로 그 증상이다.

**다만 `apps/web/src` 변경 여부는 건너뛰지 않는다.** 이건 원장 같은 휴리스틱이 아니라 도메인
제약이다 — 캡션은 스크린샷에서 나오므로(D9·D12) 화면 변경이 없으면 홍보할 것이 없다. 우회
대신 `NO_WEB_CHANGES` 사유로 돌려준다.

**요청에는 반드시 회신한다.** 성공이면 만들어진 초안 ID 를, 실패면 사유 코드를 남긴다. 회신이
빠지면 요청이 lease 만료까지 `IN_PROGRESS` 로 남아 화면에는 영영 "처리 중"만 보이는데, 조용한
0건과 똑같이 나쁘다. 사유는 **코드만** 받는다 — 예외 원문에는 diff·캡션·경로가 섞일 수 있어
`policy.SAFE_FAILURE_CODES` 의 고정 목록만 통과시키고 나머지는 예외 종류 이름으로 줄인다.

동시 점유는 Mongo `findAndModify` 단일 갱신으로 막는다(`MongoSupportNotificationOutboxAdapter`
와 같은 방식). lease 는 90분으로, 한 실행의 최악 소요(후보 3 × 15분)보다 길게 잡는다.

**D17 의 "일회성 프로세스 하나" 전제는 아직 깨지지 않는다.** 트리거가 늘었어도 실행 주체는
여전히 타이머가 띄우는 컨테이너 하나이고, systemd 가 같은 유닛의 중복 기동을 막는다. 그 전제는
T12 에서 깨지므로 배타 장치는 그때 함께 넣는다.
...
- **T12. 온디맨드 기동.** D20 의 요청은 다음 타이머 실행까지 기다린다. 즉시 시작하려면 호스트
  쪽에 짧은 주기로 도는 트리거 유닛이 필요하다 — **방향은 "호스트가 백엔드를 폴링"이어야
  한다.** 반대 방향(백엔드가 도커·systemd 를 건드림)은 compose 정책 회귀 테스트가 막고 있고,
  뚫으면 그 보호를 잃는다. 이때 **타이머 실행과 겹칠 수 있으므로**(메모리 여유 ~900MB 에 1g
  컨테이너 둘 = OOM) D17 의 "일회성 프로세스 하나" 전제가 깨진다. 배타 장치를 같은 라운드에
  넣어야 한다.
...
- `agent-worker` — `gole_agent_worker`(AgentJobs, gRPC)에 편입하지 **않기로** 한 근거는 D16에
  ...
  **2026-09-22 재검토(D20): 여전히 편입하지 않는다.** 트리거는 생겼지만 실행 주체는 아직
  타이머가 띄우는 프로세스 하나라 분산 leasing 이 풀 문제가 없다. 요청 직렬화는 Mongo
  `findAndModify` 하나로 충분했다. 더구나 D16 의 세 번째 근거(egress 가 필요한 작업을
  "문의 원문은 밖으로 안 나간다"가 설계 중심인 프로세스에 섞으면 그 보장이 약해진다)는
  조건부가 아니라 **영구적**이다. 다시 볼 지점은 T12 다.
