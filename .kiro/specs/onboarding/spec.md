...
이 스펙은 "가입 후 첫 로그인 시점에 전화번호가 없거나 미인증이면" 닉네임 입력 → 전화번호
...
```
required = !(nickname 있음 && phoneVerifiedAt 있음 && interestTags 비어있지 않음 && privacyConsentedAt 있음)
```
...
- [ ] R1. `Account`에 `nickname`, `phoneNumber`, `phoneVerifiedAt`, `interestTags`,
      `privacyConsentedAt`, `marketingConsentedAt`, `legacyExempt` 필드가 추가된다(전부
      nullable, `legacyExempt`만 boolean 기본값).
- [ ] R2. `GET /api/v1/accounts/me/onboarding`이 단계별 완료 여부와 `required`,
      `legacyExempt`를 반환한다(재개용).
- [ ] R3. `PUT /api/v1/accounts/me/onboarding/nickname`이 D9 규칙으로 검증하고, 중복이면
      거부한다.
- [ ] R4. `POST /api/v1/accounts/me/onboarding/phone/verification`이 D2/D4 규칙(형식·유일성·
      쿨다운·일일 한도)을 통과한 뒤 D3 경로로 OTP를 발송한다.
- [ ] R5. `POST /api/v1/accounts/me/onboarding/phone/verification/confirm`이 코드를 대조하고
      성공 시 `phoneVerifiedAt`을 저장한다. 5회 오답 시 해당 OTP를 무효화한다.
- [ ] R6. `PUT /api/v1/accounts/me/onboarding/interest-tags`가 D8 목록 대비 1~5개 범위를
      검증한 뒤 저장한다.
- [ ] R7. `POST /api/v1/accounts/me/onboarding/consent`가 개인정보 동의(false면 거부)와
      마케팅 동의(선택)를 받아 각각의 타임스탬프를 저장한다.
- [ ] R8. `AccountResponses.MeResponse`, `SignInResult`, `SocialLoginResult`에
      `onboardingRequired: boolean`이 추가된다. 소셜은 D7에 따라 구글만 실값, 나머지는 항상
      `false`.
- [ ] R9. 매물등록·주문생성·채팅시작 엔드포인트가 `@RequiresOnboarding` 가드를 통과해야
      한다. `legacyExempt=true`이거나 온보딩을 이미 마친 계정은 항상 통과한다. 위반 시 403
      `ONBOARDING_REQUIRED`.
- [ ] R10. 배포 시점 기존 계정 전체에 `legacyExempt=true`를 세팅하는 1회성 마이그레이션
      스크립트가 문서화된다.
- [ ] R11. 프론트 `views/onboarding`이 진입 시 R2 응답으로 이미 끝난 단계를 건너뛰고
      재개한다.
- [ ] R12. 이메일 로그인 성공(`onboardingRequired=true`)과 구글 신규가입(`newAccount=true`)
      시 `/onboarding`으로 리다이렉트된다. `legacyExempt` 계정은 리다이렉트 대신 닫을 수 있는
      배너만 본다.
- [ ] R13. 거래성 액션 호출 중 403 `ONBOARDING_REQUIRED`를 받으면 프론트 공용 API 클라이언트가
      `/onboarding`으로 유도한다.
